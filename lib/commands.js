 ...
 console.log(version); 
  if (app.argv.help) {
    print([
        ...
        ' iku version --bump --patch  - Bump the patch in version 0.0.1 -> 0.0.2',
        ' iku version --bump --minor  - Bump the minor in version 0.0.8 -> 0.1.0',
        ' iku version --bump --major  - Bump the major in version 0.9.13 -> 1.0.0',
        ''
        ].join("\n"));    
    return;
  }
  ...
  if (app.argv.bump) {
    ...
    if (app.argv.patch) { // Bump the patch number
    ...
    if (app.argv.minor) { // Bump the minor number
      version = version.split('.');
      patch = version.pop();
      patch = 0;
      minor = version.pop();
    ...
    if (app.argv.major) { // Bump the major number
      version = version.split('.');
      patch = version.pop();
      patch = 0;
      minor = version.pop();
    ...
    fs.writeFile(__dirname + '/../package.json', 
        JSON.stringify(conf, null, '\t'), function(e) {
      if (e) {
        print(e.message);
        process.exit(1);
      }
